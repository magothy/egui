...
use epaint::{Pos2, Rect, Stroke, TextShape, Shape, Color32, Rgba, emath::{NumExt, remap_clamp, round_to_decimals}};

use crate::{Response, Sense, TextStyle, Ui, Widget, WidgetText};

use super::{transform::ScreenTransform, GridMark, MIN_LINE_SPACING_IN_POINTS};
...
impl AxisConfig {
    ...
    pub fn tick_formatter(mut self, formatter: fn(f64, usize, &RangeInclusive<f64>) -> String) -> Self {
...
pub(super) struct AxisWidget {
    ...
    pub(super) rect: Rect,
    pub(super) transform: Option<ScreenTransform>,
    ...
    fn ui(self, ui: &mut Ui) -> Response {
        ...
        if ui.is_rect_visible(response.rect) {
            ...
            for step in self.steps {
                ...
                if !text.is_empty() {
                    let spacing_in_points = (transform.dpos_dvalue()[self.config.axis as usize] * step.step_size).abs() as f32;
                    ...
                    if line_alpha > 0.0 {
                        ...
                        let galley = ui.painter().layout_no_wrap(text, font_id.clone(), line_color);
                        let text_pos = match self.config.axis {
                            Axis::X => {
                                let y = match self.config.placement {
                                    AxisPlacement::Default => {
                                        self.rect.min.y
                                    }
                                    AxisPlacement::Opposite => {
                                        self.rect.max.y - galley.size().y
                                    }
                                };
                                let projected_point = super::PlotPoint::new(step.value, 0.0);
                                Pos2 {
                                    x: transform.position_from_point(&projected_point).x - galley.size().x / 2.0,
                                    y
                                }
                            }
                            Axis::Y => {
                                ...
                                Pos2 {
                                    x,
                                    y: transform.position_from_point(&projected_point).y - galley.size().y / 2.0,
                                }
                            }
                        };
